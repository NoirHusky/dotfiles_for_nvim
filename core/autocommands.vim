...
function! s:custom_highlight() abort
  ...
  " For cursor colors ( causes some weird blinking )
...
function! s:isVistaOpen() abort
  return bufwinnr('__vista__') != -1
  endif
endfunction

" Exit Vim if NERDTree is the only window remaining in the only tab.
function! s:quit_nerd_remaining() abort
	let l:nerdActive = exists('b:NERDTree') && b:NERDTree.isTabTree()
	let l:lastPage   = tabpagenr('$') == 1
	let l:isVistaActive = s:isVistaOpen()

	if l:nerdActive && l:lastPage
		if winnr('$') == 1 | quit | endif
		if winnr('$') == 2 && l:isVistaActive | quit | quit | endif
	endif
endfunction

autocmd BufEnter * call s:quit_nerd_remaining()
